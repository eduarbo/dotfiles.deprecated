;;; config.el --- pretty-symbols layer config file for Spacemacs.
;;
...
(defun add-fira-code-symbol-keywords (keywords)
             ...
             `(,(car regex-char-pair)
               (0 (prog1 ()
                    (compose-region
                     ...
                     ;; literal tab
                     ,(concat "	" (list (decode-char
                                         'ucs (cadr regex-char-pair)))))))))
           keywords)))

(add-hook 'prog-mode-hook
          (lambda () (add-fira-code-symbol-keywords
                      '(("\\(www\\)"                   #Xe100)
                        ("[^/]\\(\\*\\*\\)[^/]"        #Xe101)
                        ("\\(\\*\\*\\*\\)"             #Xe102)
                        ("\\(\\*\\*/\\)"               #Xe103)
